...
def generate_shadow_product(shape: str, segment: str, concept: str, index: int) -> dict:
    ...
    return {
        ...
        "price": price,
        "image_url": f"https://placehold.co/500x800/121212/00F5D4?text={item_name.replace(' ', '+')}",
...
async def analyze_profile(req: ScanRequest):
    ...
    if req.shoulders is None and req.category.lower() != "none":
        ...
        std_measurements = {
            "shoulders": round(h * 0.58, 1), "chest": round(h * 0.55, 1),
            "waist": round(h * 0.42, 1), "hips": round(h * 0.56, 1)
        }
    ...
    recommended_products = [item for item in CATALOG if req.segment.lower() == item["segment"].lower() and (req.concept.lower() == item["concept"].lower() or req.concept.lower() == "any") and (shape_result in item["target_shapes"] or "All" in item["target_shapes"])]

    # Guarantee 10 recommendations
    ...
    return {"shape": shape_result, "standard_metrics": std_measurements, "products": recommended_products}
